...
INTERRUPT_WAITING_FOR_MODEL_PREFIX = "操作已中断：正在等待模型响应（"
...
def _billing_or_entitlement_message(
    *,
    ...
    lines = [
        (
            f"{provider_label} 报告计费、积分或账户"
            f"权限已用完，无法使用 {model_label}。"
        ),
        "请向该提供商添加积分或更新计费，然后重试。",
    ]
    if base_url_host_matches(str(base_url or ""), "openrouter.ai"):
        lines.append("OpenRouter 积分：https://openrouter.ai/settings/credits")
    lines.append("你可以使用 /model <model> --provider <provider> 临时切换提供商。")
    return "\n".join(lines)
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        if agent._budget_grace_call:
            ...
            if not agent.quiet_mode:
                agent._safe_print(f"\n️ 迭代预算已用尽（已使用 {agent.iteration_budget.used}/{agent.iteration_budget.max_total} 次迭代）")
            break
        ...
        if _runtime_context_error:
            ...
            agent._emit_status(" Ollama 运行时上下文太小，无法使用 Hermes 工具")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        if not agent.quiet_mode:
            agent._vprint(f"\n{agent.log_prefix}🔄 正在进行 API 调用 #{api_call_count}/{agent.max_iterations}...")
            agent._vprint(f"{agent.log_prefix}    请求大小：{len(api_messages)} 条消息，约 {approx_tokens:,} tokens（约 {total_chars:,} 字符）")
            agent._vprint(f"{agent.log_prefix}    可用工具：{len(agent.tools) if agent.tools else 0}")
        else:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if not agent.quiet_mode:
                    agent._vprint(f"{agent.log_prefix}️  API 调用在 {api_duration:.2f}s 内完成")
                ...
                if response_invalid:
                    ...
                    agent._buffer_vprint(f"️  无效的 API 响应（尝试 {retry_count}/{max_retries}）：{', '.join(error_details)}")
                    agent._buffer_vprint(f"   提供商：{provider_name}")
                    cleaned_provider_error = agent._clean_error_message(error_msg)
                    agent._buffer_vprint(f"    提供商消息：{cleaned_provider_error}")
                    agent._buffer_vprint(f"   ️  {_failure_hint}")
                    
                    if retry_count >= max_retries:
                        # Try fallback before giving up
                        if agent._has_pending_fallback():
                            agent._buffer_status(f"️ 无效响应的最大重试次数（{max_retries}）已达——正在尝试备用...")
                        if agent._try_activate_fallback():
                            retry_count = 0
                            compression_attempts = 0
                            _retry.primary_recovery_attempted = False
                        ...
                        agent._flush_status_buffer()
                        agent._emit_status(f" 无效响应的最大重试次数（{max_retries}）已达，放弃。")
                    ...
                    agent._buffer_vprint(f" 正在 {wait_time:.1f}s 后重试（{_failure_hint}）...")
                    ...
                    while time.time() < sleep_end:
                        if agent._interrupt_requested:
                            agent._vprint(f"{agent.log_prefix} 重试等待期间检测到中断，正在中止。", force=True)
                            agent._persist_session(messages, conversation_history)
                ...
                if finish_reason == "length":
                    if getattr(response, "id", "") == PARTIAL_STREAM_STUB_ID:
                        agent._vprint(
                            f"{agent.log_prefix}️  流在网络错误时中断 "
                            f"（finish_reason='length'，流式传输存根）",
                            force=True,
                        )
                    else:
                        agent._vprint(
                            f"{agent.log_prefix}️  响应被截断 "
                            f"（finish_reason='length'）——模型达到最大输出 token 限制",
                            force=True,
                        )
                    ...
                    if _thinking_exhausted:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix} 推理耗尽了输出 token 预算——"
                            f"未生成可见响应。",
                            force=True,
                        )
                    ...
                    if agent.api_mode in {"chat_completions", "bedrock_converse", "anthropic_messages"}:
                        ...
                        if assistant_message is not None and _trunc_has_tool_calls:
                            ...
                            if truncated_tool_call_retries < 3:
                                ...
                                if _is_stub_stall:
                                    ...
                                    agent._buffer_vprint(
                                        f"️  流在工具调用中断——"
                                        f"正在重试（{truncated_tool_call_retries}/3）..."
                                    )
                                else:
                                    agent._buffer_vprint(
                                        f"️  检测到截断的工具调用——"
                            ...
                            if _is_stub_stall:
                                agent._vprint(
                                    f"{agent.log_prefix}️  流在3次重试后仍在工具调用中断——该操作未被执行。",
                                    force=True,
                                )
                            else:
                                agent._vprint(
                                    f"{agent.log_prefix}️  再次检测到截断的工具调用响应——拒绝执行不完整的工具参数。",
                                    force=True,
                                )
                            agent._cleanup_task_resources(effective_task_id)
                            agent._persist_session(messages, conversation_history)
                    ...
                    if len(messages) > 1:
                        agent._vprint(f"{agent.log_prefix}    回滚到最后一个完整的助手轮次")
                        ...
                        agent._flush_status_buffer()
                        agent._vprint(f"{agent.log_prefix} 第一个响应被截断——无法恢复", force=True)
                        agent._persist_session(messages, conversation_history)
                        return {
                            "final_response": None,
                            "messages": messages,
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    if getattr(agent.context_compressor, "_context_probed", False):
                        ...
                        if getattr(agent.context_compressor, "_context_probe_persistable", False):
                            ...
                            agent._safe_print(f"{agent.log_prefix}💾 已缓存的上下文长度：{agent.model} 为 {ctx:,} tokens")
                ...
                agent._vprint(f"{agent.log_prefix} API 调用期间被中断。", force=True)
                agent._persist_session(messages, conversation_history)
                interrupted = True
                final_response = f"{INTERRUPT_WAITING_FOR_MODEL_PREFIX}{api_elapsed:.1f}s 已用）。"
                break
                ...
                if isinstance(api_error, UnicodeEncodeError) and getattr(agent, '_unicode_sanitization_passes', 0) < 2:
                    ...
                    if _surrogates_found or _is_surrogate_error:
                        ...
                        if _surrogates_found:
                            agent._buffer_vprint(
                                f"️  从消息中删除了无效的代理项字符。正在重试..."
                            )
                        else:
                            agent._buffer_vprint(
                                f"️  代理项编码错误——在全载荷清理后重试..."
                            )
                        continue
                    if _is_ascii_codec:
                ...
                if (
                    ...
                    if agent._try_refresh_codex_client_credentials(force=True):
                        ...
                        agent._buffer_vprint(f" {_label} 认证在401后已刷新。正在重试请求...")
                        continue
                if (
                    ...
                    if agent._try_refresh_nous_client_credentials(force=True):
                        print(f"{agent.log_prefix} Nous 密钥在401后已刷新。正在重试请求...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix} Nous 401 — Portal 认证失败。")
                    if _body_text:
                        print(f"{agent.log_prefix}   响应：{_body_text}")
                    if not _print_nous_entitlement_guidance(agent, "Nous model access"):
                        print(f"{agent.log_prefix}   最可能的原因：Portal OAuth 已过期、账户积分不足或密钥被撤销。")
                    print(f"{agent.log_prefix}   故障排除：")
                    print(f"{agent.log_prefix}      重新认证：hermes auth add nous")
                    print(f"{agent.log_prefix}      检查积分/结算：https://portal.nousresearch.com")
                    print(f"{agent.log_prefix}      验证存储的凭证：{_dhh}/auth.json")
                    print(f"{agent.log_prefix}      临时切换提供商：/model <model> --provider openrouter")
                if (
                    ...
                    if agent._try_refresh_copilot_client_credentials():
                        agent._buffer_vprint(f" Copilot 认证在401后已刷新。正在重试请求...")
                        continue
                if (
                    ...
                    if agent._try_refresh_anthropic_client_credentials():
                        print(f"{agent.log_prefix} Anthropic 凭证在401后已刷新。正在重试请求...")
                        continue
                    # Credential refresh didn't help — show diagnostic info
                    key = agent._anthropic_api_key
                    print(f"{agent.log_prefix} Anthropic 401 — 认证失败。")
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   认证方式：Microsoft Entra ID（httpx 事件钩子）")
                        print(f"{agent.log_prefix}   运行 `hermes doctor` 获取凭证链诊断信息，或")
                        print(f"{agent.log_prefix}   `az login`（如果你的开发者会话已过期）。")
                    else:
                        auth_method = "Bearer（OAuth/setup-token）" if _is_oauth_token(key) else "x-api-key（API 密钥）"
                        print(f"{agent.log_prefix}   认证方式：{auth_method}")
                        print(f"{agent.log_prefix}   令牌前缀：{key[:12]}..." if isinstance(key, str) and len(key) > 12 else f"{agent.log_prefix}   令牌：（空或太短）")
                    print(f"{agent.log_prefix}   故障排除：")
                    from hermes_constants import display_hermes_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}      检查 {_dhh}/.env 中的 ANTHROPIC_TOKEN（Hermes 管理的 OAuth/setup 令牌）")
                    print(f"{agent.log_prefix}      检查 {_dhh}/.env 中的 ANTHROPIC_API_KEY（API 密钥或旧版令牌值）")
                    print(f"{agent.log_prefix}      对于 API 密钥：在 https://platform.claude.com/settings/keys 验证")
                    print(f"{agent.log_prefix}      对于 Claude Code：运行 'claude /login' 刷新，然后重试")
                    print(f"{agent.log_prefix}      清理旧版配置：hermes config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}      清除过期密钥：hermes config set ANTHROPIC_API_KEY \"\"")
                ...
                agent._buffer_vprint(f"️  API 调用失败（尝试 {retry_count}/{max_retries}）：{error_type}{_status_code_str}")
                agent._buffer_vprint(f"    提供商：{_provider}  模型：{_model}")
                agent._buffer_vprint(f"    端点：{_base}")
                agent._buffer_vprint(f"    错误：{_error_summary}")
                ...
                if is_rate_limited and agent._fallback_index < len(agent._fallback_chain):
                    ...
                    if not pool_may_recover:
                        if classified.reason == FailoverReason.billing:
                            agent._buffer_status(
                                "️ 计费或积分已耗尽——正在切换到备用提供商..."
                            )
                        else:
                            agent._buffer_status("️ 达到速率限制——正在切换到备用提供商...")
